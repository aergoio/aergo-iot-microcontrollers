...
int sh2lib_connect(struct sh2lib_handle *hd, const char *uri)
{
    ...
    if ((hd->http2_tls = esp_tls_conn_http_new(uri, NULL)) == NULL) {  //&tls_cfg)) == NULL) {
        ESP_LOGE(TAG, "[sh2-connect] esp-tls connection failed");
        goto error;
    }

    //if (cfg && cfg->non_block) {
        FD_ZERO(&hd->http2_tls->rset);
        FD_SET(hd->http2_tls->sockfd, &hd->http2_tls->rset);
        hd->http2_tls->wset = hd->http2_tls->rset;
